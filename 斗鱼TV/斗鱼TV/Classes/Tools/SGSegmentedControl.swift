    ...
    func setupTitleArr() {
        if self.segmentedControlType == .sgSegmentedControlTypeScroll {
            ...
            for text in self.title_Arr! {
                ...
                self.title_btn?.setTitleColor(kNormalColor, for: UIControlState())
                self.title_btn?.setTitleColor(kSelectColor, for: .selected)
            ...
            self.indicatorView?.backgroundColor = kSelectColor
